...
f0='pro_df[,k] ~ Age + Sex + Race + BMI + Smoking.Status + Occupation + Education + icu + (1 | PLATE.ID)'
f1=paste(f0, '+ case')
 
for (k in seq(1:ncol(pro_df))){
  print(k)
...
colnames(denoised)=colnames(pro_df)
rownames(denoised)=rownames(pro_df)
saveRDS(denoised, "Proteins_denoised.rds")

Table_pooled=cbind(Beta_pooled, pvalue_pooled)
rownames(Table_pooled)=colnames(X_pooled)
...
MysPLSDA_pooled <- splsda(pro_df,data$case, ncomp = 1,
                          keepX = 5)
...
var = c("eosinophil_sputum_percentage","neutrophil_sputum_percentage","fev1_percentage",
        ...
        "icu","icu_last_year","atopy","nasal_polyps","Smoking.Status","Second.Hand.Smoke","BMI","case")
data_score <-  data[,var]
